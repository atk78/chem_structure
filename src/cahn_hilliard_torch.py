        ...
        for cycle in tqdm(range(self.cycle)):
            ...
            for i in range(1, self.save_per_steps + 1):
                ...
                if step % self.gif_interval == 0:
                    if self.dim == 2:
                        tmp_fig = record.fig_2d(self.c[i].cpu().numpy(), f"{self.filename}_{step}_step")
                    elif self.dim == 3:
                        tmp_fig = record.fig_3d(self.c[i].cpu().numpy(), f"{self.filename}_{step}_step")
                    tmp_fig.savefig(
                        self.save_dir.joinpath("tmp").joinpath(f"{self.filename}_{step}_step.png")
                    )
                    tmp_fig.clear()
