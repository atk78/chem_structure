...
class CahnHilliard:
    def __init__(
        ...
        if self.dim == 2:
            self.init_c = self.c0 + noise * np.random.standard_normal((Nxy, Nxy))
            ...
            self.init_c = self.c0 + noise * np.random.standard_normal((Nxy, Nxy, Nz))
        ...
        np.save(
            self.npy_dir.joinpath("0_step.npy"), self.init_c[-1]
        )
        ...
        for cycle in tqdm(range(self.cycle)):
            ...
            for i in range(1, self.save_per_steps + 1):
                ...
                if i == self.save_per_steps:
                    ...
                    np.save(
                        self.npy_dir.joinpath(f"{cnt}_step.npy"), self.c[i]
                    )
