...
def load_npy_as_3d_texture(npy_file):
    ...
    texture_3d = pyglet.image.Texture3D.create_for_images(
        image_array, internalformat=GL_RED
    )
    return texture_3d
...
def sort_intersections(intersections, normal):
    ...
    angles = [
        np.arctan2(
            np.dot(dr0, (p - center) / np.linalg.norm(p - center)),
...
def gen_slices(shader1, x0, y0, z0, batch):
...
def show_3d_texture(npy_file_path: str):
    ...
    texture = load_npy_as_3d_texture(npy_file_path)

    # npy_file = "cahn–hilliard-model.npy"
    # texture = load_npy_as_3d_texture(3d_npy_file_path)
    ...
    # initial slices
    vertex_list = gen_slices(shader1, x0, y0, z0, batch1)
    global vertex_list
    ...
    def update(dt):
        ...
        inv_pos = model_mat.__invert__() @ Vec4(
